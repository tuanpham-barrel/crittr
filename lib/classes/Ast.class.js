...
class Ast {
    ...
    static getAstOfRuleMap(ruleMap) {
        ...
        for (let [ruleKey, rulesObj] of ruleMap) {
            rulesObj = rulesObj.filter(ruleObj => {
                return !(ruleObj.rule.hasOwnProperty('declarations') && !ruleObj.rule.declarations.length)
                    && !(ruleObj.rule.type === 'rule' && !ruleObj.rule.hasOwnProperty('declarations'));
            })
